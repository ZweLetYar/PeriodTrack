...
import { updateDocument, useGetCollection } from "../../hooks/useFireStore";
import { useNavigate } from "react-router-dom";

export default function LogPeriodForm() {
  ...
  let currentUserId = users && users[0] ? users[0].id : "DefaultId";

  let saveLog = async () => {
    if (!currentUserId || !selectedDate) {
      console.error("Missing user ID or selected date");
      return;
    }

    let data = { lastPeriodDate: selectedDate };

    try {
      await updateDocument("users", currentUserId, data);
      navigate("/");
    } catch (err) {
